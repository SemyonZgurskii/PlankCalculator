const LONG_PLANK_LENGTH = 6000;
const SHORT_PLANK_LENGTH = 3000;
const currentPlankLength = 3000;
...
.filter((it) => currentPlankLength / it >= 1)
.sort((a, b) => a - b);
// let sizesForLongPlank = [];
// let sizesForShortPlank = [];

// currentSizes.forEach((it) => {
//   if (currentPlankLength / it > 1) {
//     sizesForShortPlank.push(it);
//   } else {
//     sizesForLongPlank.push(it);
//   }
// })

// sizesForLongPlank.sort((a, b) => a - b);
...
function getIndexForSumm (array, currentSize) {
  ...
  array.forEach((it, index) => {
    if (it <= (currentPlankLength - currentSize)) {
      indexForSumm = index;
    }
  })
  return indexForSumm;
...
while (suitableSizes.length > 0) {
  ...
  newArray[newArrayIndex].push(currentElement);

  if (indexForSumm >= 0) {
    while (indexForSumm >= 0) {
      newArray[newArrayIndex].push(suitableSizes[indexForSumm]);
    ...
    remains += currentPlankLength - sizesSumm;
  } else {
    remains += currentPlankLength - currentElement;
    suitableSizes.splice(currentIndex, 1);
  }
  plankCount += 1;
  newArrayIndex += 1;
...
console.log(remains, plankCount, '/--/', newArray);
...
// После прогона первого цикла , в котором был подбор длинн для 3 метровых заготовок, изъять элементы (массивы), сумма длинн в которых будет меньше определенного значения. Далее длины из этих элементов использовать для подбора в цикле для 6 метровых заготовок.Попробовать запоминать элементы с маленькой суммой длинн в ходе первого цикла для трехметровых заготовок.
